...
class SubtitleGeneratorGUI:
    def __init__(self, root):
        ...
        self.speaker_threshold = 0.8  # 说话人相似度阈值
        self.min_speaker_samples = 3  # 最少说话人样本数
    ...
    def auto_detect_speakers(self):
        ...
        unassigned_speaker_id = 0

        # 对每个字幕片段进行说话人识别
        for i, subtitle in enumerate(self.subtitles_with_speakers):
            # 更新进度
            progress = 10 + int(80 * (i / total_segments))
            self.update_progress(progress, f"分析语音片段 {i+1}/{total_segments}...")
            ...
            # 尝试分类说话人
            speaker = self.classify_speaker(embedding)
            if not speaker:
                # 新说话人（自动命名）
                # 新说话人（自动命名为“说话人X”，确保序号连续）
                unassigned_speaker_id += 1
                # 检查是否已有相同序号的“说话人X”，如果有则递增序号
                while f"说话人{unassigned_speaker_id}" in self.speakers:
                    unassigned_speaker_id += 1
                speaker = f"说话人{unassigned_speaker_id}"
            ...
            # 更新特征库和字幕数据
            self.update_speaker_embeddings(speaker, embedding)
            self.subtitles_with_speakers[i]['speaker'] = speaker
        ...
        messagebox.showinfo("成功", f"已完成说话人识别，共识别出 {len(self.speaker_embeddings)} 个说话人")
